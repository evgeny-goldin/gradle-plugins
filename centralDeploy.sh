...
# 3) 'sonatype-nexus-snapshots'/'sonatype-nexus-staging' credentials in "$M2_HOME/conf/settings.xml"
...
for module in 'about' 'common' 'crawler' 'duplicates' 'gitdump' 'monitor' 'node' 'play' 'teamcity'
do
    cd "$rootDir/$module"
    echo "[<===[`pwd`]===>]"
