...
class AboutTask extends BaseTask
{
    ...
    */
    ...
    /**
     * Retrieves current plugin convention object.
     * @return current plugin convention object
     */
    private AboutPluginConvention c() { ( AboutPluginConvention ) project.convention.plugins[ 'about' ] }
    ...
    void taskAction()
    {
        final directory = c().directory ?: jarTask.destinationDir
        final fileName  = c().fileName  ?: "about-${group}-${name}-${version}.txt"
        final split     = { String s -> ( List<String> )( s ? s.split( /,/ ).toList()*.trim().findAll{ it } : null ) }
        final files     = files( directory, split( c().include ), split( c().exclude ))
        final tempFile  = new File( jarTask.temporaryDir, fileName )
        final prefix    = (( c().prefix == '/' ) ? '' : c().prefix )
    ...
    String buildContent ()
    {
        ...
        | ${ c().dumpPaths ? 'Directory     : [' + props[ 'user.dir' ] + ']': '' }
        | Java          : [${ props[ 'java.version' ] }][${ props[ 'java.vm.vendor' ] }]${ c().dumpPaths ? '[' + props[ 'java.home' ] + ']' : '' }[${ props[ 'java.vm.name' ] }]
        | OS            : [${ props[ 'os.name' ] }][${ props[ 'os.arch' ] }][${ props[ 'os.version' ] }]
        |===============================================================================
        | Gradle Info
        |===============================================================================
        | ${ c().dumpPaths ? 'Home          : [' + project.gradle.gradleHomeDir.canonicalPath + ']' : '' }
        | ${ c().dumpPaths ? 'Basedir       : [' + rootDir.canonicalPath + ']': '' }
        | ${ c().dumpPaths ? 'Build file    : [' + project.buildFile.canonicalPath + ']' : '' }
        | GRADLE_OPTS   : [${ env[ 'GRADLE_OPTS' ] ?: '' }]
        | Version       : [${ project.gradle.gradleVersion }]
        | Project       : [${ c().dumpPaths ? project.toString() : project.toString().replaceAll( /\s+@.+/, '' )}]
        | Tasks         : ${ project.gradle.startParameter.taskNames }
        | Coordinates   : [$group:$name:$version]
        | ${ c().dumpDependencies ? 'Dependencies  : [' + padLines( dependenciesContent()) + ']' : '' }""" +
    ...
    String scmContent()
    {
        ...
        {
            gitStatusCommand = 'git status' + ( c().gitStatusProject ? '' : ' ' + rootDir.canonicalPath )
    ...
    String gitContent( String gitStatus )
    {
        ...
        | ${ c().gitStatusProject ? 'Project' : 'Basedir' } Status: [${ padLines( gitStatus ) }]
