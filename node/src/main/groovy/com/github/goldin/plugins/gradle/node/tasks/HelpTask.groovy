...
class HelpTask extends NodeBaseTask
{
    ...
    void taskAction()
    {
        ...
        System.out.println( "- gradle $SETUP_TASK        : installs Node.js and all application dependencies, runs automatically" )
        ...
        System.out.println( "- gradle $CHECK_STARTED_TASK : checks that application started is up and running, runs automatically after '$START_TASK' and '$RESTART_ALL_TASK'" )
        System.out.println( "- gradle $STOP_TASK         : stops an application, runs automatically before '$START_TASK' and if '$CHECK_STARTED_TASK' fails" )
        System.out.println( "- gradle $STOP_ALL_TASK      : stops all currently running applications" )
        System.out.println( "- gradle $CHECK_STOPPED_TASK : checks that application stopped is not accepting connections any more, runs automatically after '$STOP_TASK' and '$STOP_ALL_TASK'" )
    }
}
